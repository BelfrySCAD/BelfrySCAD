...
The debugger runs the evaluator in a daemon worker thread (`DebugSession`) and surfaces a single shared `DebuggerPane` (owned by `MainWindow`, not per-tab) with call-stack and variables panels. `_debug_session` and `_debug_tab` on `MainWindow` track the active session and the `FileTab` that started it. The viewport, console, and animate pane are all window-level singletons — there is no per-tab routing. See openscad_cpp_evaluator's `CLAUDE.md` for the evaluator internals referenced below.
...
`partial_bodies`/`partial_error` (Phase 3 — see "Live partial-tree rendering" below) are the result of `_generate_partial_render(generate_partial)`, called right before every emit of `paused`/`error_break`.
...
**Fast continue (bytecode VM)** — the hook's `set_fast_continue` callback (present whenever the evaluator supports it) tells `Evaluator` it's safe to speed up function calls via the compiled bytecode VM instead of the plain interpreter for whatever runs next, checked per-function/per-call against `Evaluator.chunkEligibleNow()` rather than a blanket switch — see openscad_cpp_evaluator's `CLAUDE.md` for the bytecode VM/interpreter split itself. `_apply_fast_continue()` (called from both of `hook()`'s return points) passes the current breakpoint dict when it's safe, or `None` to force full interpretation. It's disabled only while a `step_into`/`step_to_child` command is active (both need the very next statement-level checkpoint, possibly one level deeper than the step started, so every call in between must stay interpreted) or while `_break_on_first`/`_pause_requested` are set. Plain `continue`, `step_over`, and `step_out` never examine anything deeper than the step's own starting depth, so whatever they run through — including a long computation being stepped over — is eligible for the VM; a breakpoint inside that stepped-over call is still honored, since eligibility is re-checked per call rather than assumed for the whole step.
...
The hook needs no reference to the evaluator and keeps no tree bookkeeping of its own: `debug_evaluate()` passes the hook a `generate_partial` callable (openscad_cpp_evaluator's `Evaluator::generatePartialTree()`), which reads the C++ evaluator's own in-progress tree stack directly, flattened across every nesting level. See openscad_cpp_evaluator's `CLAUDE.md` for the two-pass resolve/generate design this depends on: the partial tree can be generated at any point, since resolve never depends on a node's generated bodies.

`_generate_partial_render(generate_partial)` (module-level function in `debugger.py`) calls that callable for whatever has been resolved so far, converts the result via `bodies_from_dicts()`, and returns `(bodies, None)` on success or `(None, str(e))` if a generate step raises. Called from both `hook()` and `_error_break()`, right before every `paused`/`error_break` emit — so **every** pause (breakpoint hit, Step Into/Over/Out, the Pause button, and error breaks) triggers a live regeneration, not just step commands that cross a module-call boundary. `MainWindow` passes `self._csg_cache` (a `ManifoldCache`, see openscad_cpp_evaluator's `CLAUDE.md`) to `DebugSession(manifold_cache=...)`, which hands it to `debug_evaluate()`, so partial generation here benefits from the same content-hash cache the main render path uses, with no debugger-specific caching code of its own: within one debug session the evaluator's tree persists across consecutive pauses, so every previously-stepped-past subtree becomes a guaranteed cache hit on later pauses — the cost no longer grows with the tree across a session the way it did before this cache existed.

**Generates from every level of the evaluator's in-progress tree stack flattened together, not just the finished top-level tree.** This flattening happens inside `Evaluator::generatePartialTree()` on the C++ side; nothing here has to reach into evaluator internals for it. A `CSGNode` only gets appended to its parent's accumulator once that parent's own resolve step returns — so for a script whose whole geometry is one deeply-nested top-level statement (e.g. `difference(){union(){cube();sphere();}cylinder();}`), the top-level tree stays completely empty for the *entire* time spent stepping through `cube()`/`sphere()`/`cylinder()`, since `difference()`'s own `CSGNode` isn't appended anywhere until every child (transitively) has finished resolving. Using only the top-level tree here would mean the live preview never shows anything until the whole script finishes — exactly backwards from the point of this feature. Flattening every level of the stack picks up already-resolved leaves (e.g. `cube()`'s `CSGNode` sitting in `union()`'s still-in-progress accumulator) regardless of how many enclosing statements haven't finished yet.
...
**Camera position on pause**: `_on_debug_paused` only reapplies `$vpt`/`$vpr`/`$vpd`/`$vpf` to the viewport camera for names the innermost frame's `dyn_explicit` set says the *script itself* assigned (the `dyn_explicit` set the evaluator puts on each frame — `EvalContext::dynExplicit` in openscad_cpp_evaluator's `include/openscad_cpp_evaluator/eval_context.hpp`) — not just any `$vp*` name present in scope, which would always be true since they're pre-seeded from the current camera at debug start (`MainWindow._viewport_params`). Without this distinction, every single pause would silently snap the camera back to wherever it was when the debug session started, discarding any manual orbit/pan/zoom the user did while stepping through — the seeded value looks identical to an explicit one by simple dict-membership, so the fix has to track "explicitly assigned" separately from "merely present."
...
Implementation: `Evaluator._check_debug` stashes `self._last_children_positions` — the `(origin, line)` pairs of the paused `ModularCall` node's own top-level, non-declaration children — right before invoking the debug hook, via `Evaluator._child_statement_positions(node)`. This is deliberately *not* threaded through the `debug_hook` callback's own signature (which would require updating every hand-rolled test hook in `tests/test_debugger.py`); instead, `debug_evaluate()` passes the hook a `get_children_positions` callable, which `hook()` calls at resume time, capturing the result into `self._step_to_child_targets` — the same callable-passing pattern as `generate_partial` in the live partial-render feature above. `hook()`'s step dispatch gains a `"to_child"` branch: `step_hit = (resolved_origin, line) in self._step_to_child_targets or depth < self._step_depth` (both gated by `not expr_level`) — the first clause catches whichever child is reached first in execution order (e.g. a module calling `children(1)` before `children(0)`), the second is the call-return fallback.
