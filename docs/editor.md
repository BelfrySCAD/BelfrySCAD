...
- **Viewport**: `_GridViewport(_SimpleViewport)`. Quad mesh faces (triangulated, yellow at 50% alpha both sides) with black edge lines between adjacent grid points. The semi-transparent faces use standard alpha blending (`SRC_ALPHA, ONE_MINUS_SRC_ALPHA`), enabled per-buffer in `_SimpleViewport._paintGL_inner` when `buf["color"][3] < 1.0`. Vertex markers are depth-tested (occluded by mesh). Green octahedrons for unselected, red↔white blink (250ms) for selected. Hovering shows `[row,col]: (x, y, z)` tooltip. Clicking a vertex switches the dropdown to that row and selects the vertex in the table.
- **Faces**: checkbox (checked by default) switches between face mode and skeleton mode. Face mode renders triangulated quads with black edge lines (the existing look). Skeleton mode (`draw_faces=False`) renders only row lines (blue, `[0.15, 0.45, 0.85]`) and column lines (orange, `[0.85, 0.45, 0.1]`) via `upload_lines`, with no fill — useful for seeing through the surface or inspecting the grid's parametric structure.
