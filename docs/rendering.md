...
The Export dialog's default filename. `belfryscad/export_name.py` holds the whole thing — pure and Qt-free, so the GUI, the debugger and the headless CLI all seed it identically; a script reading `$export_name` should not find it defined in one and undefined in another.

**No evaluator change was needed.** `viewport_params` seeds arbitrary `$`-names (the name is historical, not a restriction — `evaluateImpl` does a plain `for (k, v) : viewportParams` into `ctx.dyn`), and `Evaluator.dyn` hands every `$`-variable back after `evaluate()`. So the feature is a seed on the way in and a read on the way out.

`_RenderWorker` seeds from `self._file_path`, the **original** path — never `parse_path`, which for an unsaved buffer is a temp file whose name would leak into the dialog. The final value rides back on `finished` as an eighth element and is stashed per `FileTab`, so switching tabs offers the right name.

Read back **unconditionally**, unlike the `$vp*` values immediately above it, which are filtered through `dyn_explicit`. Those are pre-seeded from the live camera and go stale if the user orbits mid-render; `$export_name` is seeded from a file path that cannot change under it, and the seeded value *is* the wanted default when the script leaves it alone.

Sanitising keeps `[A-Za-z0-9_+.-]` and replaces every other character with one underscore — per character, not per run, so length stays recognisable. Path separators are in the replaced set, so a script cannot steer the dialog out of its directory.
...
**Cut faces**: a face `difference()` exposes takes the cutting object's colour, or the cut green (#9DCB51, what OpenSCAD's preview and colour-preserving render show) when the cutter has no colour — so a cut through an uncoloured part still reads as a cut. Preferences ▸ Viewport ▸ **Cut faces** flips that to the colour of the object that was cut (`viewport/keepMinuendColor` → `Evaluator(keep_minuend_color=True)`); the evaluator differences each minuend part on its own so a multi-colour model's cross-section keeps every part's colour. See openscad_cpp_evaluator's `CLAUDE.md` on `keepMinuendColor`.
...
**Axis ticks and labels**: each axis has perpendicular tick marks (X/Z ticks extend along Y; Y ticks extend along X). Ticks are one-sided, extending only in the positive perpendicular direction; minor ticks are ~24 px, major ticks ~48 px. The world-space half-length `L` driving both the drawn axis-line extent and `_nice_spacings(L)`'s tick/label spacing target (`raw = L / 15`) comes from `_axis_extent(camera)` (free function, `renderer.py`, pure math, unit-tested in `test_renderer.py::TestAxisExtent`): `distance * tan(fov/2) / tan(DEFAULT_FOV/2) * 2.5` — scaled by `fov` as well as `distance` so that changing `fov` alone (Shift+wheel, no change in `distance`) keeps tick density visually consistent on screen; previously this was a flat `distance * 2.5`, so narrowing `fov` (optical zoom-in) left tick spacing unchanged in world-space while far fewer ticks fit on screen, and widening `fov` left the axes looking sparse. Dividing by `tan(DEFAULT_FOV/2)` makes this identical to the old flat formula at the default fov, so views that never touch `$vpf` are unaffected. When only one minor tick would fall between majors (`major_steps <= 2`), spacing is promoted so the minor interval becomes the new major and the old major becomes the label interval, keeping the axis visually dense. That promotion lives in `_drawn_spacings(L)`, not in `_render_axes`, because the label walker needs the *promoted* numbers too: **every major tick carries a label**, so a label placed from `_nice_spacings`' un-promoted tier would land where no major tick was drawn. Tick labels are depth-aware, camera-facing textured billboards rendered in the 3D scene (not a QPainter screen-space overlay) — each `QImage` is rasterized once into a ModernGL texture and drawn as a unit quad oriented via `right`/`up` vectors from the view matrix. Labels are positioned on the negative perpendicular side (opposite the ticks). Labels and minor ticks on axes within 5° of end-on to the camera are suppressed (major ticks still draw). Before that hard cutoff, `_axis_density(camera)` (free function, `renderer.py`, pure math — no GL/Camera-instance coupling beyond taking a `Camera` argument, unit-tested in `test_renderer.py::TestAxisDensity`) thins labels and minor ticks per axis to avoid the alternative: evenly-world-spaced labels crowding into an ever-smaller on-screen span as the axis foreshortens, overlapping illegibly well before 5°. For each axis, `view_dir[ai]` (cosine of the angle between the camera's view direction and that world axis) gives foreshortening `sqrt(1 - view_dir[ai]**2)` — 1 when the axis is broadside (full density), shrinking toward 0 as it swings end-on. Half the reciprocal (doubling density vs. a plain 1/foreshorten stride), snapped to the nearest power of two, is the "show every Nth tick" stride — powers of two keep the visible subset nested as the camera rotates (every 4th-tick set ⊂ every-2nd ⊂ every tick) rather than jumping to an unrelated set each frame. `_render_axes` applies this stride via `_tick_is_drawn(k, major_steps, end_on, stride)` (pure, unit-tested in `test_renderer.py::TestTickIsDrawn`): at the hard end-on cutoff, only major ticks draw (unchanged, long-standing behavior — majors always show even dead-on end-on). Below that cutoff, stride applies uniformly to major *and* minor ticks alike, rather than exempting majors the way the hard cutoff does — `major_steps` (how often a major tick falls) and `stride` aren't related, so exempting majors there let an every-major-step tick land arbitrarily close to a kept minor tick while a stride-sized gap opened up elsewhere, a visibly inconsistent spacing bug. Requiring just `k % stride == 0` (majors included) keeps the drawn set a strict, evenly-spaced arithmetic subsequence regardless of where `major_steps` falls. `_axis_tick_world_points` applies the same per-axis stride to labels independently, since labels are keyed to the label-spacing tier (which may differ from the tick marks' major/minor tiers, not to the tick-mark loop's own index) — that loop only ever had the one tier, so it was never susceptible to this bug. Positive axis lines are colored (red/green/blue); negative axes are gray.

**XY grid**: one line per minor tick across the XY plane, spanning the same `L` as the axes, in `grid_color` — a derived property, `GRID_AXES_MIX = 0.25` of the way from `bg_color` to `axes_color`. Derived rather than stored so a colour-scheme change carries it along; there is no fixed colour that stays legible against one theme's background without vanishing into another's axes. Toggled by **View ▸ Show Grid** (Ctrl+G, `SceneRenderer.show_grid`), independently of Show Axes — it is the one overlay dense enough to want gone while keeping the axes and their labels, and for that same reason it is **off by default**.

⚠️ Two defaults have to stay in step: `SceneRenderer.show_grid` and the "Show Grid" menu item's initial checked state. `_add_checkable` sets its checkbox *before* connecting the toggle slot, so a menu default never reaches the renderer — a mismatch shows an unchecked menu item over a visible grid, with the first Ctrl+G appearing to do nothing. `tests/test_export_name.py::TestGridDefaultsOff` pins that they agree. Data-viewer dialogs embed the same renderer and so inherit the same default.

Built in `_render_axes` but drawn as its **own VAO, before the axes, with `_active_fbo.depth_mask = False`**, and this is load-bearing rather than tidiness. Grid lines lie in the same `z=0` plane as the axes and their tick marks, and a tick runs *along* a grid line, so the two are exactly coincident. Ordering within a single draw call does not settle that: a grid line spans `-L..+L` while a tick spans a few pixels, and interpolating depth across such different primitive lengths leaves the shared pixels differing in the last bits either way — z-fighting, seen as ticks dissolving into the grid. Drawing the grid with depth writes off keeps it out of the depth buffer entirely, so the ticks drawn afterwards always win, while the grid is still depth-*tested* and stays correctly hidden behind solid geometry. No grid line is emitted at 0, since that is the axis itself.
